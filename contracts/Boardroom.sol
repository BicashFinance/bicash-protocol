...
contract Boardroom is ShareWrapper, ContractGuard, Operator {
    ...
    function claimReward() public updateReward(msg.sender) {
        ...
        if (reward > 0) {
            ...
            directors[msg.sender].epochTimerStart = getCurrentEpochTimestamp();
            uint256 scale = IBasisAsset(address(cash)).scale();
            cash.safeTransfer(msg.sender, reward.div(scale));
            emit RewardPaid(msg.sender, reward);
        }
    }

    function allocateSeigniorage(uint256 _amount)
        external
        onlyOneBlock
        onlyOperator
    {
        ...
        uint256 amount = _amount.mul(IBasisAsset(address(cash)).scale());
